...
# define serialDebug(fmt, args...)  if (FastSerial::getInitialized(0)) do {Serial1.printf("%s:%d: " fmt "\n", __FUNCTION__, __LINE__ , ##args); delay(0); } while(0)
#else
...
void AP_Param::show_all(void)
{
    ...
    for (ap=AP_Param::first(&token, &type);
        ...
        switch (type) {
        case AP_PARAM_INT8:
            Serial1.printf_P(PSTR("%s: %d\n"), s, (int)((AP_Int8 *)ap)->get());
            break;
        case AP_PARAM_INT16:
            Serial1.printf_P(PSTR("%s: %d\n"), s, (int)((AP_Int16 *)ap)->get());
            break;
        case AP_PARAM_INT32:
            Serial1.printf_P(PSTR("%s: %ld\n"), s, (long)((AP_Int32 *)ap)->get());
            break;
        case AP_PARAM_FLOAT:
            Serial1.printf_P(PSTR("%s: %f\n"), s, ((AP_Float *)ap)->get());
            break;
        default:
            break;
        }
